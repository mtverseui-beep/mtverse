...
function createGenericSeededSocial(slug: string): { purchaseCount: number; reviews: TemplateReview[] } {
  ...
  return {
    ...
    reviews: [0, 1, 2].map((offset) => ({
      ...
      date: new Date(Date.UTC(2026, 5, Math.max(1, 18 - offset * 4), 10 + offset, 15)).toISOString(),
      verifiedPurchase: false,
      source: 'internal',
    })),
  }
