...
type TemplateForm = {
  ...
  pricingTier: DashboardKit['pricingTier']
  previewPath: string
...
function formToKit(form: TemplateForm, fallback?: DashboardKit): Partial<DashboardKit> {
  ...
  const originalPriceUsd = Number(form.originalPriceUsd)
  const framework = form.category === 'html' ? 'html' : fallback?.framework || 'nextjs'
  return {
    ...
    priceUsd: Number.isFinite(priceUsd) ? priceUsd : 12,
    originalPriceUsd: Number.isFinite(originalPriceUsd) ? originalPriceUsd : 49,
    pricingTier: form.isFree ? 'normal' : form.pricingTier,
    framework,
    frameworkLabel: framework === 'html' ? 'HTML/CSS/JavaScript' : 'Next.js App Router',
...
function formFromJsonRecord(record: TemplateJsonRecord, fallback?: DashboardKit): TemplateForm {
  ...
  return {
    ...
    subcategory: stringFromUnknown(record.subcategory, base.subcategory),
    ...
    pricingTier: record.pricingTier === 'pro' ? 'pro' : base.pricingTier,
...
export default function AdminTemplatesReal({ initialKits }: Props) {
  ...
  const categoryOptions = useMemo(() => {
    const categories = new Map<string, string>([['dashboard-kits', 'Dashboard Kits'], ['dashboards', 'Dashboards'], ['admin', 'Admin'], ['saas', 'SaaS'], ['ecommerce', 'Ecommerce'], ['analytics', 'Analytics'], ['crm', 'CRM'], ['finance', 'Finance'], ['marketing', 'Marketing'], ['html', 'HTML'], ['landing', 'Landing']])
  ...
  const validationItems = useMemo(() => [
    ...
    { label: 'Package key', ok: Boolean(form.packageKey.trim()) },
