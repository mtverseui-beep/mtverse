...
const packageRoot = resolve(process.argv[2] || '.work-mtadmin/out')
const packages = [
  { id: 'mtadmin-nextjs', filename: 'mtadmin-nextjs.zip' },
  { id: 'mtadmin-react', filename: 'mtadmin-react.zip' },
  { id: 'mtadmin-bundle', filename: 'mtadmin-all-frameworks-bundle.zip' },
]
...
for (const item of packages) {
  const archivePath = resolve(packageRoot, item.filename)
  ...
  if (Number(uploaded.ContentLength || 0) !== details.size) {
    throw new Error('Uploaded object size verification failed for ' + item.filename + '.')
  }

  console.log('Uploaded and verified ' + key + ' (' + details.size + ' bytes).')
}
