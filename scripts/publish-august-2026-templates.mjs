...
const slugs = ['bloom', 'aisle', 'cadence', 'ion']
const publicBucket = required('CLOUDFLARE_R2_BUCKET')
const packageBucket = required('CLOUDFLARE_R2_PACKAGE_BUCKET')

if (publicBucket === packageBucket) {
  throw new Error('Preview and private package buckets must be different.')
}
...
const artifacts = slugs.flatMap((slug) => [
  {
    slug,
    type: 'preview',
    bucket: publicBucket,
    key: `templates/dashboard-previews/${slug}/${slug}-${releaseDate}.png`,
  ...
  },
  {
    ...
    key: `templates/dashboards/${slug}/${slug}-${releaseDate}.zip`,
...
for (const artifact of artifacts) {
  ...
  if (!fileStats.isFile() || fileStats.size === 0) {
    throw new Error(`Artifact is empty: ${artifact.file}`)
  }
